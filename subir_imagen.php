...
?>
*/
/*PARA NUEVAS VERSIONES DE PHP*/
<?php
$mi_usuario='root';
$mi_password='tupassword';
	...
	if(is_uploaded_file($_FILES['foto']['tmp_name'])){
		...
		if (move_uploaded_file($_FILES['foto']['tmp_name'], $imagen_subida)) {
			...
			$link = mysqli_connect('localhost', $mi_usuario, $mi_password)
				or die('Uyy!!!: ' . mysqli_error());
			mysqli_select_db($link,'test') or die('No pudo selecionar la BD');
			//Creamos nuestra consulta sql
			$query="insert into producto(codigo, descripcion, imagen) value ('$codigo', '$descripcion', '$imagen_subida')";
			//Ejecutamos la consutla
			mysqli_query($link,$query) or die('Error al procesar consulta: ' . mysqli_error());
